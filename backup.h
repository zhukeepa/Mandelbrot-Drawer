...
//Bitmaps
BITMAP bmMandelbrot;                         //Bitmap with picture of Mandelbrot set. 
HDC hDcMem; 
...
} DrawingInfo(0, 128, 128, false, true, 1, 500, false);
...
{
    /*
        Draws the Mandelbrot set in the given columns. 
    */
    ////  Set variables  
    ...
    for (unsigned short x = StartColumn; x <= EndColumn; ++x)
    {
        for (unsigned short y = 0; y < MAIN_WINDOW_HEIGHT; ++y)
        {
            ...
            SetPixel(hDc, x, y, RGB(Sat, Sat, Sat)); 
        }
    } 
